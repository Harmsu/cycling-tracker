...
router.get('/', async (req, res) => {
  ...
  const { rows: currentRows } = await pool.query(
    'SELECT SUM(km) as km FROM rides WHERE substr(date, 1, 4) = $1', [String(year)]
  );
...
router.put('/', async (req, res) => {
  ...
  await pool.query(
    `INSERT INTO goals (year, goal_km) VALUES ($1, $2)
     ON CONFLICT (year) DO UPDATE SET goal_km = EXCLUDED.goal_km`,
